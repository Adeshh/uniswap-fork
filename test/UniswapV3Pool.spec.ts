...
import { TestDefiedgeV3SwapPay } from '../typechain/TestDefiedgeV3SwapPay'
...
import { TestDefiedgeV3ReentrantCallee } from '../typechain/TestDefiedgeV3ReentrantCallee'
...
describe('DefiedgeV3Pool', () => {
  ...
  let factory: DefiedgeV3Factory
  let pool: MockTimeDefiedgeV3Pool

  let swapTarget: TestDefiedgeV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeDefiedgeV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  // https://github.com/Defiedge/Defiedge-v3-core/issues/214
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestDefiedgeV3ReentrantCallee')
      ).deploy()) as TestDefiedgeV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestDefiedgeV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestDefiedgeV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestDefiedgeV3SwapPay
