...
import '../../../../../contracts/DefiedgeV3Pool.sol';
...
contract E2E_mint_burn {
    ...
    DefiedgeMinter minter;
    DefiedgeSwapper swapper;
    ...
    constructor() public {
        ...
        Defiedge = new SetupDefiedge(token0, token1);

        minter = new DefiedgeMinter(token0, token1);
        swapper = new DefiedgeSwapper(token0, token1);
    ...
    function check_mint_invariants(
        int24 _tickLower,
        int24 _tickUpper,
        DefiedgeMinter.MinterStats memory bfre,
        DefiedgeMinter.MinterStats memory aftr
    ) internal {
        (, int24 currentTick, , , , , ) = pool.slot0();
    ...
    function _init(uint128 _seed) internal {
        ...
        // deploy the pool
        //
        Defiedge.createPool(poolParams.fee, poolParams.startPrice);
        pool = Defiedge.pool();
    ...
    function test_mint(uint128 _amount) public {
        ...
        (DefiedgeMinter.MinterStats memory bfre, DefiedgeMinter.MinterStats memory aftr) =
    ...
    function test_burn_partial(uint128 _amount) public {
        ...
        try minter.doBurn(pos.tickLower, pos.tickUpper, burnAmount) returns (
            DefiedgeMinter.MinterStats memory bfre_burn,
            DefiedgeMinter.MinterStats memory aftr_burn
        ) {
            bfre = bfre_burn;
            aftr = aftr_burn;
        } catch {
